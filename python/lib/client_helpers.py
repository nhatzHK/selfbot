...
async def purge(msg, client):
    deleted = await client.purge_from \
...
async def challenge(msg, client):
    chal = "Wame is awesome"
    it = 4
    await client.send_message(msg.channel, 'Type {} {} times.'.format(chal, it))
    for i in range(4):
        msg = await client.wait_for_message(author = msg.author, content = chal)
        fmt = '{} left to go...'
        await client.send_message(msg.channel, fmt.format(it - i -1))

    await client.send_message(msg.channel, \
            'Not so dumb, heh?\nI mean great job wumpus!')

# Stop sending messages for a given time
async def pause(msg, client):
    t = 5
    await asyncio.sleep(10)
    await client.send_message(msg.channel, 'Came back, has the Jedi!')

# Count the number of messages a user has in a channel
async def count(msg, client):
    counter = 0
    tmp = await client.send_message(msg.channel, 'Calculating messages...')
    logs = client.logs_from(msg.channel, limit = 10000)
...
async def clean(msg, client):
    ...
    await client.send_message \
           ...
           (count, to_del_count))
    for r in nr:
        await client.delete_role(msg.server, r)
    await client.send_message \
...
async def get_xkcd(phrase, index, refs):
    ...
    If clienth failed proceed to normal search,
